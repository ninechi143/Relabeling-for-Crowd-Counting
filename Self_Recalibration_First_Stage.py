...
first_write_epoch = 60
rewrite_epoch = 10
scale = np.array([5])



train_procedure = [("part_B_final","train_data")]
test_procedure = [("part_B_final","test_data")]
...
def Training(fold = None , percentage = None , config = None):
    ...
    with tf.compat.v1.Session() as sess:
        ...
        best_MAE = 200
        best_MSE = 200
        f = open("track_cost_{0}.txt".format(str(fold)), "a")
        
        
        train_writer = tf.compat.v1.summary.FileWriter(".\\logs\\train",sess.graph)
        test_writer  = tf.compat.v1.summary.FileWriter(".\\logs\\test" ,sess.graph)
...
if __name__ == "__main__" :
    ...
    config = [True , "adap" , "V2" , [0.25 , 0.5] , 2.0]
    

    for i in range(5):

        if os.path.isdir(".\\model_last_epoch_for_inference_" + str(i)):
            pass
        else:
            os.makedirs(".\\model_last_epoch_for_inference_" + str(i))

    
        Training(fold = i , percentage = percentage , config = config)
